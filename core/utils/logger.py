...
MAX_REDACTION_DEPTH = 5  # Maximum recursion depth for sensitive field redaction
...
class ECSJsonFormatter(logging.Formatter):
    ...
    REDACTED_KEYS = {"password", "token", "secret", "authorization", "api_key", "access_key", "auth", "credentials"}
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        Returns:
            JSON-formatted string compliant with ECS schema.
        """
    ...
    def _redact_sensitive_fields(self, log_entry: dict[str, Any], depth: int = 0) -> None:
        ...
        """
        if depth > MAX_REDACTION_DEPTH:  # Prevent deep recursion
            return
...
def setup_logging(level: str | None = None) -> None:
    ...
    logger = logging.getLogger(__name__)
    logger.info(
        "Structured logging configured: level=%s, service=%s, env=%s, version=%s",
        level,
        Constants.ServiceIdentity.name,
        Constants.ServiceIdentity.env,
        Constants.ServiceIdentity.version,
    )
