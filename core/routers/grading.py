...
import logging
from models import UnitSolution, LearningUnit
from schema import (
    QuizSubmissionRequest, QuizSubmissionResponse, QuizResultItem,
    CodeVerificationRequest, CodeVerificationResponse
)
...
async def submit_quiz(request: QuizSubmissionRequest) -> QuizSubmissionResponse:
    ...
    if not solution or not solution.quiz_answers:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Quiz answers not found for this unit"
        )
    ...
    for quiz_id, selected_answer in request.answers.items():
        ...
        if not correct_answer:
            raise HTTPException(
                ...
                detail=f"Invalid quiz ID: {quiz_id}"
            )
...
@router.post("/code/verify", response_model=CodeVerificationResponse)
async def verify_code(request: CodeVerificationRequest) -> CodeVerificationResponse:
    """
