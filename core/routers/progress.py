...
async def update_progress(request: ProgressUpdateRequest) -> ProgressUpdateResponse:
    ...
    existing = await UserProgress.find_one(UserProgress.user_id == request.user_id, UserProgress.unit_id == unit.id)
...
async def get_user_progress(user_id: str) -> UserProgressResponse:
    ...
    progress_records = await UserProgress.find(UserProgress.user_id == user_id).to_list()

    if not progress_records:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No progress found for user")
