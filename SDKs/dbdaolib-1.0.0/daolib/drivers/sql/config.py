...
from daolib.constants import DatabaseType, DatabaseSchema
...
class DbConnectionEntry:
    ...
    def __post_init__(self):
        ...
        if normalized_drive not in valid_types:
            raise ValueError(
                f"Unsupported database type: {self.system}. "
        ...
        if self.server_selection_timeout_ms <= 0:
            raise ValueError(
                f"server_selection_timeout_ms must be > 0, "
                f"got {self.server_selection_timeout_ms}"
            )
        
        # SSL/TLS validation
        if self.use_ssl and self.ssl_verify and self.ssl_ca_path:
            if not Path(self.ssl_ca_path).exists():
                raise ValueError(
                    f"SSL CA certificate path does not exist: {self.ssl_ca_path}"
                )
        ...
        if normalized_drive == DatabaseType.SQLITE.value:
            if self.port != 0:
                raise ValueError(
                    f"SQLite does not use port numbers. Got: {self.port}. "
                    "Set port=0."
                )
            if self.username or self.password:
                raise ValueError(
                    "SQLite does not support authentication. "
                    "username and password must be empty."
                )
            if not self.path:
                raise ValueError(
                    "SQLite requires a 'path' parameter pointing to the database file. "
            ...
            if not self.username or not self.password:
                raise ValueError(
                    f"{self.system} requires username and password credentials"
                )
    ...
    def connection_string(self) -> str:
        """
        Builds a SQL connection string with safe URL encoding.
        ...
        if normalized_drive == DatabaseType.POSTGRESQL.value:
            ...
            raise ValueError(f"Unsupported database type: {normalized_drive}")
    ...
    def build_ssl_query_params(self) -> str:
        """
        Builds optional SSL/TLS query parameters for the connection string.
