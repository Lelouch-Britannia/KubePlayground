...
class SQLQueryLogger:
    ...
    def before_cursor_execute(self, _conn, _cursor, _statement, _parameters, context, _executemany):
    ...
    def after_cursor_execute(self, _conn, _cursor, statement, parameters, context, executemany):
        ...
        LogBuilder(logger).event(LogEvent.SQL_QUERY).success().msg(f"Query: {operation}").duration_ms(
            duration_ms
        ).db_context(system=db_system, database=self.database, host=self.host_label, role=self.role).field(
            "db.statement_hash", stmt_hash
        ).field("db.operation", operation).field("db.param_count", len(parameters) if parameters else 0).field(
            "db.executemany", executemany
        ).level(logging.DEBUG).emit()
    ...
    def handle_error(self, exception_context):
        ...
        LogBuilder(logger).event(LogEvent.SQL_QUERY).failure(error_code, exc).msg(
            f"Query failed: {operation}"
        ).duration_ms(duration_ms).db_context(
            system=db_system, database=self.database, host=self.host_label, role=self.role
        ).field("db.statement_hash", stmt_hash).field("db.operation", operation).field(
            "db.param_count", len(parameters) if parameters else 0
        ).emit()
...
class SQLDriver(AbstractDBDriver):
    ...
    def __init__(
        ...
        secondary_config: DbConnectionEntry | None = None,
        *,
        ...
        self.write_engine: Engine | None = None
        self.read_engine: Engine | None = None

        # Query logging setup
        self._enable_query_logging = enable_query_logging
        self._primary_query_logger: SQLQueryLogger | None = None
        self._secondary_query_logger: SQLQueryLogger | None = None

    def connect(self) -> tuple[Engine, Engine | None]:
        ...
        try:
            ...
            LogBuilder(logger).event(LogEvent.SQL_PING).success().msg(
                "Primary SQL engine connectivity verified"
            ).duration_ms(ping_duration_ms).db_context(
                system=self.primary_config.system,
                database=self.primary_config.database,
                host=self.primary_config.safe_host_label(),
                role="primary",
            ).level(logging.INFO).emit()

            # Pool ready event
            total_duration = (time.perf_counter() - start_time) * 1000
            builder = (
                LogBuilder(logger)
                .event(LogEvent.SQL_POOL_READY)
                .success()
            ...
            if self.secondary_config is not None:
                ...
                LogBuilder(logger).event(LogEvent.SQL_PING).success().msg(
                    "Secondary SQL engine connectivity verified"
                ).duration_ms(sec_ping_duration_ms).db_context(
                    system=self.secondary_config.system,
                    database=self.secondary_config.database,
                    host=self.secondary_config.safe_host_label(),
                    role="replica",
    ...
    def _add_pool_fields(self, builder, config: DbConnectionEntry):
        ...
        """
        return (
            builder.field("db.pool.size", config.pool_size)
    ...
    def _log_connection_error(self, error_code: InfraErrorCode, exc: Exception, role: str) -> None:
        ...
        LogBuilder(logger).event(LogEvent.SQL_INIT).failure(error_code, exc).msg(
            f"{role.capitalize()} SQL engine initialization failed"
        ).db_context(system=config.system, database=config.database, host=config.safe_host_label(), role=role).emit()

    def _cleanup_engines(self, write_engine: Engine | None, read_engine: Engine | None) -> None:
    ...
    def disconnect(self) -> None:
        ...
        if self.write_engine is not None:
            try:
                ...
                LogBuilder(logger).event(LogEvent.SQL_CLOSE).success().msg("Primary SQL engine closed").db_context(
                    system=self.primary_config.system,
                    database=self.primary_config.database,
                    host=self.primary_config.safe_host_label(),
                    role="primary",
                ...
                LogBuilder(logger).event(LogEvent.SQL_CLOSE).failure(InfraErrorCode.UNKNOWN_FATAL, e).msg(
                    "Primary SQL engine close failed"
                ).db_context(
                    system=self.primary_config.system,
                    database=self.primary_config.database,
                    host=self.primary_config.safe_host_label(),
        ...
        if self.read_engine is not None and self.secondary_config is not None:
            try:
                ...
                LogBuilder(logger).event(LogEvent.SQL_CLOSE).success().msg("Secondary SQL engine closed").db_context(
                    system=self.secondary_config.system,
                    database=self.secondary_config.database,
                    host=self.secondary_config.safe_host_label(),
                    role="replica",
                ...
                LogBuilder(logger).event(LogEvent.SQL_CLOSE).failure(InfraErrorCode.UNKNOWN_FATAL, e).msg(
                    "Secondary SQL engine close failed"
                ).db_context(
                    system=self.secondary_config.system,
                    database=self.secondary_config.database,
                    host=self.secondary_config.safe_host_label(),
