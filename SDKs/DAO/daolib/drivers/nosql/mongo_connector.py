...
class MongoConnector(AbstractNoSQLConnector):
    ...
    def __init__(self, document_models: list[Any] | None = None):
        # Singleton Protection: Only initialize once
        if not hasattr(self, "_bootstrapped"):
            self._client: AsyncIOMotorClient | None = None
            self._driver: MongoDriver | None = None
            self._config: NoSQLConnectionEntry | None = None
    ...
    async def init(self) -> None:
        ...
        async with self._init_lock:
            ...
            # Temp state holders
            config: NoSQLConnectionEntry | None = None
            client: AsyncIOMotorClient | None = None
            driver: MongoDriver | None = None
            ...
            try:
                ...
                LogBuilder(logger).event(LogEvent.MONGO_PING).success().msg("MongoDB ping successful").duration_ms(
                    ping_duration
                ).db_context(
                    system="mongodb",
                ...
                LogBuilder(logger).event(LogEvent.MONGO_INIT).success().msg(
                    "MongoDB connected successfully"
                ).duration_ms(duration).db_context(
                    system="mongodb",
                    database=config.database,
                    host=config.safe_host_label,
                    **{
                ...
                if self._document_models:
                    if not BEANIE_INSTALLED:
                        LogBuilder(logger).event(LogEvent.MONGO_ODM_INIT).msg(
                            "Models provided but 'beanie' not installed. Skipping ODM init."
                        ).level(logging.WARNING).field("models_count", len(self._document_models)).emit()
                        return

                    try:
                        odm_start = time.perf_counter()
                        await init_beanie(
                            database=self.get_database(),  # type: ignore
                            document_models=self._document_models,
                        )
                        self._beanie_bound = True

                        LogBuilder(logger).event(LogEvent.MONGO_ODM_INIT).success().msg(
                            "Beanie ODM initialized"
                        ).duration_ms((time.perf_counter() - odm_start) * 1000).db_context(
                            system="mongodb",
                            database=config.database,
                            host=config.safe_host_label,
                            **{
                        ...
                        ).field("models_count", len(self._document_models)).emit()
            ...
            # ---------------------------------------------------------
                ...
                self._handle_critical_failure(
                    client, e, InfraErrorCode.CONF_INVALID, "Configuration Error", MongoConnectionException
                )
            except ServerSelectionTimeoutError as e:
                self._handle_critical_failure(
                    client, e, InfraErrorCode.NET_TIMEOUT, "Connection Timeout", MongoConnectionException
                )
            except OperationFailure as e:
                self._handle_critical_failure(
                    client, e, InfraErrorCode.AUTH_FAILURE, "Authentication/Operation Failed", MongoConnectionException
                )
            except ConnectionFailure as e:
                self._handle_critical_failure(
                    client, e, InfraErrorCode.NET_UNREACHABLE, "Network Unreachable", MongoConnectionException
                )
            except Exception as e:
                if isinstance(e, DaoException):
                    raise  # Re-raise if it's already our custom type
                self._handle_critical_failure(
                    client, e, InfraErrorCode.UNKNOWN_FATAL, "Unexpected Initialization Error", MongoConnectionException
                )
    ...
    async def register_models(self, additional_models: list[Any], *, env_restriction: bool = True) -> None:
        ...
        if not BEANIE_INSTALLED:
            raise MongoODMException(InfraErrorCode.ODM_INIT_FAIL, "Beanie not installed. Cannot register models.")
        ...
        new_models = [model for model in additional_models if model.__name__ not in existing_model_names]
        ...
        try:
            ...
            LogBuilder(logger).event(LogEvent.MONGO_ODM_INIT).success().msg(
                "Additional Beanie models registered"
            ).duration_ms((time.perf_counter() - odm_start) * 1000).db_context(
                system="mongodb",
            ...
            ).field("new_models_count", len(new_models)).field("total_models_count", len(self._document_models)).field(
                "new_model_names", [m.__name__ for m in new_models]
            ).emit()
            ...
            LogBuilder(logger).event(LogEvent.MONGO_ODM_INIT).failure(InfraErrorCode.ODM_INIT_FAIL, e).msg(
                "Failed to register additional models"
            ).field("attempted_models", [m.__name__ for m in new_models]).emit()
    ...
    def get_client(self) -> AsyncIOMotorClient:
        if not self._client:
            raise MongoConnectionException(
                InfraErrorCode.NET_UNREACHABLE, "Client not initialized. Call await connector.init() first."
            )
        return self._client
    ...
    def close(self) -> None:
        ...
        if self._client:
            try:
                self._client.close()
                LogBuilder(logger).event(LogEvent.MONGO_CLOSE).success().msg("MongoDB connection closed").field(
                    "db.host", getattr(self._config, "safe_host_label", "unknown")
                ).emit()
            except Exception as e:
                LogBuilder(logger).event(LogEvent.MONGO_CLOSE).failure(InfraErrorCode.UNKNOWN_FATAL, e).msg(
