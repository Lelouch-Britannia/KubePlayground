...
class AbstractNoSQLConnector(ABC):
    ...
    def read_and_load_config(self) -> NoSQLConnectionEntry:
        """[Helper Responsibility] Load configuration from external sources.
    ...
    async def init(self) -> None:
        """[Connector Responsibility] Idempotent async initialization.
    ...
    def get_client(self) -> Any:
        """Return the underlying raw driver client (e.g. AsyncIOMotorClient).
    ...
    @abstractmethod
    def close(self) -> None:
        """Close connections and reset the singleton state.
