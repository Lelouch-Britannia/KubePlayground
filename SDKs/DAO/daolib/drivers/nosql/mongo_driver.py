...
from daolib.drivers.nosql.config import AbstractDBDriver, NoSQLConnectionEntry
...
class QueryLogger(monitoring.CommandListener):
    ...
    def started(self, event: monitoring.CommandStartedEvent) -> None:
        ...
        Args:
            event: PyMongo command started event.
        """
    ...
    def succeeded(self, event: monitoring.CommandSucceededEvent) -> None:
        ...
        Args:
            event: PyMongo command succeeded event.
        """
        ...
        LogBuilder(logger).event(LogEvent.MONGO_QUERY).success().msg(f"Query: {event.command_name}").duration_ms(
            duration_ms
        ).db_context(system="mongodb", database=self.db_name, host=self.host_label).field(
            "db.statement_hash", stmt_hash
        ).field("db.operation", event.command_name).level(logging.DEBUG).emit()

    def failed(self, event: monitoring.CommandFailedEvent) -> None:
        """Log query failure.

        Args:
            event: PyMongo command failed event.
        """
        duration_ms = (time.perf_counter() - self._start_times.pop(event.request_id, time.perf_counter())) * 1000
        ...
        LogBuilder(logger).event(LogEvent.MONGO_QUERY).failure(InfraErrorCode.QUERY_SYNTAX, exc).msg(
            f"Query failed: {event.command_name}"
        ).duration_ms(duration_ms).db_context(system="mongodb", database=self.db_name, host=self.host_label).field(
            "db.statement_hash", stmt_hash
        ).field("db.operation", event.command_name).emit()
...
class MongoDriver(AbstractDBDriver):
    ...
    def connect(self) -> AsyncIOMotorClient:
        """Create and return the Motor Client.
        ...
        try:
            ...
            if self._enable_query_logging:
                self._query_logger = QueryLogger(db_name=self.config.database, host_label=self.config.safe_host_label)
            ...
            LogBuilder(logger).event(LogEvent.MONGO_INIT).success().msg("Motor client created").db_context(
                system="mongodb",
            ...
            LogBuilder(logger).event(LogEvent.MONGO_INIT).failure(InfraErrorCode.CONF_INVALID, e).msg(
            ...
            LogBuilder(logger).event(LogEvent.MONGO_INIT).failure(InfraErrorCode.UNKNOWN_FATAL, e).msg(
                "Unexpected error creating Mongo Client"
            ).db_context(
                system="mongodb",
                database=self.config.database,
                host=self.config.safe_host_label,
    ...
    def disconnect(self) -> None:
        ...
        if self.client:
            try:
                self.client.close()
                LogBuilder(logger).event(LogEvent.MONGO_CLOSE).success().msg("Mongo client closed").field(
                    "db.host", self.config.safe_host_label
                ).emit()
            except Exception as e:
                LogBuilder(logger).event(LogEvent.MONGO_CLOSE).msg("Error closing Mongo client").field(
                    "error.message", str(e)
                ).level(logging.WARNING).emit()
            finally:
                self.client = None
